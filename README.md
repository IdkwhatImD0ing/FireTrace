...
- A language-neutral **REST API** ([docs/api.md](docs/api.md), OpenAPI at `/api/v1/openapi.json`): `POST /api/v1/traces` records a trace, complete in one request or **streamed as it runs** (a start without `endedAt`, then `POST /api/v1/traces/{id}/spans` as spans finish and `POST /api/v1/traces/{id}/end`), so a crash halfway still leaves the trace with every span that finished; every step retries idempotently. `POST /api/v1/traces/{id}/scores` attaches a rating, verdict, or eval result; `PATCH /api/v1/traces/{id}` merges free-form keys into that trace's `metadata`; `GET` routes list and read traces, scores, and the project; `DELETE` removes a trace or a score explicitly.
...
- A small **TypeScript SDK** (`packages/sdk-js`) that streams each trace as it happens (start, span batches, end), with retries, redaction, content truncation, safe error capture, and a read client for the API that also adds scores and patches trace metadata.
...
- A **trace page** with a span tree and duration waterfall (search by span name, collapse subtrees, tokens and cost inline on each span), an inspector that shows everything about the selected trace or span on one scrolling panel (input, output, error, details, metadata, attributes, events, scores) and renders chat-shaped input and output as messages with a JSON toggle, newer/older navigation in the list's order (`[` and `]`), and a canonical-JSON download.
...
- Spans are immutable once sent, and a trace is immutable once it has ended, apart from `metadata`; re-sending a trace or span id with different content is rejected with `409`. A streamed trace that never receives its end request stays `running` for good: nothing closes it automatically (that would be an age-based mutation), it is not counted in the dashboard's numbers, and only an owner can delete it. Spans are sent when they finish, so a hard kill mid-span loses that one span. `PATCH /api/v1/traces/{id}` merges into `metadata` only, shallowly and last-writer-wins: no history, no conflict detection, and `bodyHash` keeps describing the body as ingested rather than the document as it stands. `metadataUpdatedAt` is the only marker that a trace was edited afterwards.
