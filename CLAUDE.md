...
- **Data model:** `projects/{projectId}` (24 hex), top-level `apiKeys/{keyId}` (HMAC digest only; plaintext `ft_live_<16hex>_<64hex>` shown once; `scopes` from `scopes.ts` — `traces:write|read|delete`, legacy docs without `scopes` = write-only; optional `expiresAt`; throttled `lastUsedAt`; optional `environment` slug from `environment.ts`, null = unassigned, editable via `setApiKeyEnvironment`), `projects/{id}/traces/{traceId}` (32 hex, immutable except `metadata`, which `PATCH` merges into via `metadata.ts`, stamping `metadataUpdatedAt` and leaving `bodyHash` as ingested; `bodyHash` for idempotency; `environment` + `keyId` copied from the key at ingest, never from the body and never rewritten except by `scripts/backfill-environment.ts`), `.../spans/{spanId}` (16 hex), `projects/{id}/scores/{scoreId}` (16 hex; typed post-hoc judgements via `scores.ts`, append-only, names `^[A-Za-z0-9_-]{1,64}$`; the trace doc keeps the newest per name in its `scores` map; score bytes count on trace + project so `deleteTrace` cascades cleanly; no environment on scores, it is resolved through the trace). Indexes in `firestore.indexes.json`; large payload fields are exempted.
- **Dashboard rollups (`src/lib/firetrace/stats-rollup.ts`, `stats.ts`):** `projects/{id}/stats/{YYYY-MM-DD}` day docs (UTC) get `FieldValue.increment` deltas inside the ingest, score and delete transactions (map keys are base64url names, `_other` past the per-day caps, latency as 48 half-octave histogram buckets); `projects/{id}/statsByEnv/{env}:{YYYY-MM-DD}` twins carry the same deltas per environment (`_unassigned` for null). `getProjectStats(db, id, range, environment?)` reads ≤90 docs by document-id range from one collection or the other and folds them for `/projects/[id]/dashboard`. `scripts/backfill-stats.ts` rebuilds both (dry run unless `--apply`). Never aggregate over `traces` at read time.
- **Environment selector (`src/lib/environment-selection.ts`):** the dashboard's choice lives in the `firetrace_env` cookie (`all` | `unassigned` | slug), written by `EnvironmentSelector` and read by `getEnvironmentView()` (per-request cached; defaults to `production` when a key has it). Every project page, the CSV export and the bulk evaluator action apply `view.filter` on top of the URL filters; the URL never carries the environment.
- **Evaluators (`src/lib/eval/`, `docs/evaluators.md`):** `schema.ts` (Zod evaluator definition, `EVAL_LIMITS`), `templates.ts` (built-in prompt templates, data only), `prompt.ts` (render `{{input}}`-style variables, judge messages, JSON-schema response format, parse/coerce the verdict), `llm.ts` (fetch-only OpenAI-compatible `chat/completions` client, one retry, never logs prompts), `evaluators.ts` (Firestore CRUD for `projects/{id}/evaluators` + `evalRuns` log), `run.ts` (`runEvaluator`/`runEvaluatorBulk`/`previewEvaluator`: skip unless `force`, fall back without `response_format` on 400, write a `source: "eval"` score). Config is the optional trio `FIRETRACE_EVAL_BASE_URL/API_KEY/MODEL` (`env.eval`, all-or-nothing). Owner-only: every evaluator action checks `owner.role === "owner"`; trial users never see the controls. Runs are manual (trace page, bulk over the filtered list, capped at 50); on-ingest rules are a later phase.
...
- **Key-authenticated API** (`docs/api.md`, OpenAPI from `openapi.ts` at `/api/v1/openapi.json`): `POST/GET /api/v1/traces`, `GET/PATCH/DELETE /api/v1/traces/[traceId]`, `POST/GET /api/v1/traces/[traceId]/scores`, `DELETE .../scores/[scoreId]`, `GET /api/v1/scores`, `GET /api/v1/project`, `GET /api/v1/key`. **MCP** (`docs/mcp.md`): `packages/mcp-server` (`@firetrace/mcp`, workspace package transpiled by Next) exports `createFireTraceMcpServer(backend)` + `TraceBackend`; the app serves it statelessly at `POST /api/mcp` over `src/lib/mcp/firestore-backend.ts`; the stdio bridge uses `HttpBackend` against the REST API. Tools are registered per scope. Integration tests call the handlers directly via `tests/integration/api-helpers.ts`.
- **Docs site:** `/docs` and `/docs/[slug]` (`src/app/docs`, public, rendered per request so the header can show the visitor's session) render `docs/*.md` through `src/lib/docs` (registry → `parseMarkdown` → React; no Markdown dependency, no raw HTML). Add a new doc to `DOCS` in `registry.ts` **and link it from the introduction page**; relative `.md` links are rewritten to `/docs` routes. `/docs` itself is a hand-written introduction (`src/app/docs/page.tsx`) whose copy-paste samples live in `src/app/docs/samples.ts`; its payloads are typed against `schema.ts` and validated in `tests/unit/docs-samples.test.ts`.
- **Other modules:** `projects.ts` (project/key CRUD, cascading deletes), `queries.ts` (cursor-paginated trace list, filters `status|model|name|tag|environment|sessionId|userId|from|to`, `sort` `newest|slowest|costliest` (the latter two only with status/model/name/tag/environment; cursors carry the sort), page params `after|before`, `recentFacets()` for datalists; `parseTraceFilters`/`parseScoreFilters`/`parseTraceSort` take `{ strict: true }` on the API so an unknown value is a 400, and `requireKnownParams()` in `api-handler.ts` rejects unknown query parameters — the dashboard stays lenient), `storage.ts` (estimate + 70%/90% warnings), `sample.ts` (deterministic sample trace), `csv.ts` (owner-only CSV export of the current list page at `/api/projects/[projectId]/traces/export`), `chat-format.ts` (detect chat-shaped input/output for the inspector's message view), `tree.ts` (`buildSpanTree` + `visibleRows` for search/collapse in `TraceExplorer`), `src/lib/log.ts` (JSON logs with a secret/content denylist). Trace links carry the list's filters + sort (`traceListQuery`) so the trace page can offer newer/older neighbours via `listTraces` with `limit: 1`.
- **Next.js 16 conventions:** read `AGENTS.md` and `node_modules/next/dist/docs/`. `params`/`searchParams` are Promises; `PageProps`/`RouteContext` types are generated by `pnpm typegen`. `loading.tsx` lives only in leaf page segments (route groups such as `(traces)` for index pages), and every `notFound()` for a missing project or trace runs in a layout above all loading boundaries, so the HTTP status is a real 404 (see `loading.md` → Status Codes in the Next docs).
- **Docs to keep accurate:** `README.md`, `docs/*.md`, `src/app/docs/samples.ts`, `packages/sdk-js/README.md`, `CONTRIBUTING.md`, `SECURITY.md`.
...
- `pnpm exec tsx scripts/backfill-stats.ts --project <id> [--apply]` — rebuild the dashboard's per-day rollups (all-environment and per-environment) from stored traces and scores (needed once for projects that predate the dashboard or environments).
- `pnpm exec tsx scripts/backfill-environment.ts --project <id> [--environment <slug> --key <keyId> --tag <tag> --from <iso> --to <iso> --overwrite] [--apply]` — mark pre-environment traces as unassigned (no `--environment`) or classify history by key/tag/time, then rebuild rollups. Dry run unless `--apply`. Only when explicitly asked.
- `pnpm exec tsx scripts/backfill-feedback-metadata.ts --project <id>` — one-off migration of legacy `metadata.feedbackFor` stand-in traces into their target's metadata. Dry run unless `--apply`. Only when explicitly asked.
