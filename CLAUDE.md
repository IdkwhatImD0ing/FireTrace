...
- **Docs site:** `/docs` and `/docs/[slug]` (`src/app/docs`, public, statically generated) render `docs/*.md` through `src/lib/docs` (registry → `parseMarkdown` → React; no Markdown dependency, no raw HTML). Add a new doc to `DOCS` in `registry.ts`; relative `.md` links are rewritten to `/docs` routes.
