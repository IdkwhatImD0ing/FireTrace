...
- **Data model:** `projects/{projectId}` (24 hex), top-level `apiKeys/{keyId}` (HMAC digest only; plaintext `ft_live_<16hex>_<64hex>` shown once), `projects/{id}/traces/{traceId}` (32 hex, immutable, `bodyHash` for idempotency), `.../spans/{spanId}` (16 hex). Indexes in `firestore.indexes.json`; large payload fields are exempted.
...
- `pnpm sdk:typecheck` / `pnpm sdk:build` — the SDK has its own tsconfig; the root tsconfig excludes `packages`.
...
- Don't loosen `firestore.rules` (deny all). Don't import `firebase-admin` in client components, middleware/proxy, or Edge code. Don't bypass `requireOwner()`/`authenticateApiKey()`.
