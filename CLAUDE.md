...
- **Data model:** `projects/{projectId}` (24 hex), top-level `apiKeys/{keyId}` (HMAC digest only; plaintext `ft_live_<16hex>_<64hex>` shown once; `scopes` from `scopes.ts` — `traces:write|read|delete`, legacy docs without `scopes` = write-only; optional `expiresAt`; throttled `lastUsedAt`), `projects/{id}/traces/{traceId}` (32 hex, immutable except `metadata`, which `PATCH` merges into via `metadata.ts`, stamping `metadataUpdatedAt` and leaving `bodyHash` as ingested; `bodyHash` for idempotency), `.../spans/{spanId}` (16 hex), `projects/{id}/scores/{scoreId}` (16 hex; typed post-hoc judgements via `scores.ts`, append-only, names `^[A-Za-z0-9_-]{1,64}$`; the trace doc keeps the newest per name in its `scores` map; score bytes count on trace + project so `deleteTrace` cascades cleanly). Indexes in `firestore.indexes.json`; large payload fields are exempted.
- **Dashboard rollups (`src/lib/firetrace/stats-rollup.ts`, `stats.ts`):** `projects/{id}/stats/{YYYY-MM-DD}` day docs (UTC) get `FieldValue.increment` deltas inside the ingest, score and delete transactions (map keys are base64url names, `_other` past the per-day caps, latency as 48 half-octave histogram buckets); `getProjectStats()` reads ≤90 docs by document-id range and folds them for `/projects/[id]/dashboard`. `scripts/backfill-stats.ts` rebuilds them (dry run unless `--apply`). Never aggregate over `traces` at read time.
...
- `pnpm exec tsx scripts/backfill-stats.ts --project <id> [--apply]` — rebuild the dashboard's per-day rollups from stored traces and scores (needed once for projects that predate the dashboard).
