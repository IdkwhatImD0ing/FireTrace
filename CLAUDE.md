...
- Don't loosen `firestore.rules` (deny all). Don't import `firebase-admin` in client components, middleware/proxy, or Edge code. Don't bypass `requireOwner()`/`withApiKey()`; every new key-authenticated route names the scope it needs.
