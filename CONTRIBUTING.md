...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unit        | `tests/unit/**/*.test.ts`, `packages/*/src/**/*.test.ts`                                                                       | `pnpm test` (no network)                                                                    | Schema validation and normalization, canonical hashing, id rules, API-key hashing and timing-safe verification, span tree, storage math, env parsing, SDK behavior, score and evaluator schemas, judge prompt rendering and parsing, the fetch-based LLM client, dashboard rollup math |
| Integration | `tests/integration/**/*.test.ts`                                                                                               | `pnpm test:integration` (inside `firebase emulators:exec`, serial files, 30 s test timeout) | Ingestion end to end, idempotency and counters, key revocation, cross-project isolation, deletion of descendants, deny-all rules, score routes and cascades, evaluator runs with a fake judge, per-day rollups and their backfill                                                      |
| Browser     | `playwright.config.ts` + `tests/e2e/**/*.spec.ts` (excluded from the root `tsconfig.json`; Playwright type-checks them itself) | `pnpm test:e2e` (`playwright test`)                                                         | Sign-in through the Auth emulator, project and key flows, filters and pagination, trace inspector, scores, evaluator definitions, the dashboard                                                                                                                                        |
