...
Requests for one trace never overlap or reorder: they run through one queue per trace. If the start is rejected (an invalid key, a trace id already taken), the error is reported once through `onError` (or thrown from `end()` with `throwOnError`), later batches are dropped, and `end()` returns that error. A span batch that fails after its retries is reported through `onError` (with or without `throwOnError`, since nothing awaits it) and does not stop the trace: later batches and the end are still sent, and `end()` reflects the end request alone. The end body, including `endedAt`, is frozen the moment `end()` is called, so batches still draining ahead of it do not stretch the trace's duration.

Streaming needs a FireTrace server that serves `POST /api/v1/traces/{id}/spans` and `/end`. Against an older deployment the start is rejected with `400 invalid_trace` (it lacks `endedAt`) and every trace is lost, so upgrade the server first or run with `streaming: false` until then.
