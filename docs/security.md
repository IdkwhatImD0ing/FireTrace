...
- Every request re-verifies the cookie with `verifySessionCookie(value, true)` and re-checks the allowlist (`getOwner`, `requireOwner`); `getOwner` is memoized per request with React `cache()`, so a layout and its page share one verification instead of repeating it. The dashboard layout (`src/app/(dashboard)/layout.tsx`), server actions (`src/lib/actions.ts`), and the export route all go through these helpers. A cookie's presence proves nothing by itself.
...
- Exposure: trial users can read only their own data, but their traces live in the owner's Firestore and count against its quota (at most `limit × 2 MiB` of trace data per account: a whole trace is one request of at most 2 MiB, and a streamed trial trace may not grow past 2 MiB in total). Keep the limit small and treat the instance as public when the variable is set.
