...
| `list_traces`          | `status`, `model`, `name`, `tag`, `environment`, `sessionId`, `userId`, `from`, `to`, `sort`, `limit` (default 20), `cursor` | One line per trace (id, time, status, duration, name, model, environment, span/error counts) plus a cursor. `sort` is `newest`, `slowest` or `costliest`; `environment` is a slug or `unassigned`; `status` includes `running` for streamed traces that have not ended. |
...
| `record_trace`         | `trace` (the trace object; the server adds `schemaVersion: 1`)                                                               | Stored / already stored, trace id, span count. Validation errors come back as tool errors. Omitting `endedAt` stores a running trace that the REST API's spans/end requests complete.                                                                                   |
